...
- **Assets/**: 모든 게임 에셋의 메인 개발 디렉토리
  ...
  - `Scripts/`: 시스템별로 정리된 ~50개의 C# 파일:
    - `Battle/`: BattleSetup, BattleTurnSystem, ChampionAI, MonsterController
      - `BattleManager/`: BattleManager
      - `BattleEntity/`: BattleEntity, BattleEntityAnimator, BattleEntityVisual, BattleSkillExecutor
    - `Champion/`: Champion, ChampionSpawner, ChampionPathfinder, FatigueEffect
    ...
    - `UI/`: CurrencyUI, VictoryUI, DefeatUI, GameOverUI, WarningMessageUI, SettingsUI, FadeEffect, LobbyUI, MonsterDragHandler, MonsterInRoomDragHandler, MonsterInventoryUI, MonsterTooltip, ReputationUI, RoomSelectionPanel, ShopMonsterPanel, ShopUI
    - 루트: GameManager, GameStateManager, AudioManager, SaveState
  - `Prefabs/`: 6개의 UI 프리팹 (MonsterInventoryItem, MonsterStatusDisplayPrefab, RoomTypeItem, ShopMonsterPanel, SkillButtonPrefab, allyButtonPrefab)
  - `Animation/`: 스프라이트 애니메이션 (Fire, Heal, Slash)

### 게임 시스템 (구현 완료)
1. **던전 건설 시스템**: 그리드 기반 방 배치
   - 방 타입: Battle, Treasure, Boss, Entrance, Fire(불꽃), Ice(얼음), Lock(잠금), Key(열쇠)
   - **Lock/Key 메커니즘**: 열쇠(Key) 방 진입 시 연결된 잠금(Lock) 방이 전투방으로 전환됨
...
- **게임 페이즈**: `Preparation`(준비) → `Exploration`(탐험) 2단계 (GameStateManager가 관리)
  - 전투는 Exploration 페이즈 내에서 방 진입 시 BattleManager가 독립적으로 처리
...
기능 구현 시:
1. `.claude/skills/dungeon-mental-model/SKILL.md` 멘탈 모델의 의사결정 플로우를 먼저 거친다
2. `Assets/Scripts/` 하위의 적절한 디렉토리에 스크립트 생성
...
- `docs/2D-턴제-디펜스-게임-멘탈모델.md` - 아키텍처, 디자인 패턴 6가지, 개발 방법론 상세 문서
- `.claude/skills/dungeon-mental-model/SKILL.md` - 기능 구현 시 패턴 선택 가이드 (스킬로 자동 적용)
